...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    const copy: IPost = Object.assign({}, post, {
      createdDate: post.createdDate && post.createdDate.isValid() ? post.createdDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        post.createdDate = post.createdDate ? moment(post.createdDate) : undefined;
      });
    }
    return res;
  }
